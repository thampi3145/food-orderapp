...
const API_URL = `${env.PORT}`;
import getServer  from 'config/serverroutes';
...
const fetchGirabuyComponent = (path, userOptions = {}) => {
  ...
  // Build Url
  const url = `${ API_URL}${getServer.getServerApi(path.apiaction)}`;
...
function getGirabuyDetails(selectid) {
  return fetchGirabuyComponent(selectid);
}
